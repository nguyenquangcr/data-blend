...
import { backNode, IconCopyDataOpe, IconSqlOpe } from '../../constant';
import './styleHanle.scss';
import clsx from 'clsx';
import DeleteIcon from '@mui/icons-material/Delete';

const CustomNodeApi = ({ data, selected, id }) => {
  //select
  const { diDetail, opeSelect } = useSelector(state => state.project);
  ...
  return (
    <Box
      className={clsx(
        opeSelect == id ? classes.active : '',
        classes.customLabelNode
      )}
      sx={{
        maxWidth: 200,
        maxHeight: 140,
      ...
      // onClick={e => dispatch(getOperatorDetalLocal(data))}
    >
      <Handle
        style={{ zIndex: '10' }}
      ...
      <Handle
        ...
        position="right"
      />
      <CardMedia alt="Copy data">
        <MIcon
          size={'30'}
          color=""
          src={data.type == 'ftelsqlNode' ? IconSqlOpe : IconCopyDataOpe}

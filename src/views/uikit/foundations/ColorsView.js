import { Icon } from '@iconify/react';
import Page from '~/components/Page';
import React, { useState } from 'react';
// import { useSnackbar } from 'notistack';
import { PATH_APP } from '~/routes/paths';
import copyFill from '@iconify-icons/eva/copy-fill';
import { CopyToClipboard } from 'react-copy-to-clipboard';
import HeaderDashboard from '~/components/HeaderDashboard';
import { useTheme, hexToRgb } from '@mui/material/styles';
import { makeStyles } from '@mui/styles';
import {
  Box,
  Card,
  Tooltip,
  IconButton,
  Container,
  Typography
} from '@mui/material';

// ----------------------------------------------------------------------

const COLORS = ['primary', 'info', 'success', 'warning', 'error'];
const COLORS_VARIATION = ['lighter', 'light', 'main', 'dark', 'darker'];
const GREY_VARIATION = [
  '100',
  '200',
  '300',
  '400',
  '500',
  '600',
  '700',
  '800',
  '900'
];

const useStyles = makeStyles(theme => ({
  root: {},
  margin: {
    marginBottom: theme.spacing(5)
  },
  wrapper: {
    display: 'flex',
    flexWrap: 'wrap',
    margin: theme.spacing(1.5, -1.5, 0)
  },
  card: {
    width: '100%',
    position: 'relative',
    margin: theme.spacing(1.5),
    [theme.breakpoints.up('sm')]: { width: 'calc((100%/2) - 24px)' },
    [theme.breakpoints.up('md')]: { width: 'calc((100%/4) - 24px)' },
    [theme.breakpoints.up('lg')]: { width: 'calc((100%/5) - 24px)' }
  },
  copyButton: {
    position: 'absolute',
    top: theme.spacing(1),
    right: theme.spacing(1)
  }
}));

// ----------------------------------------------------------------------

function ColorCard({ hexColor, variation, onCopy }) {
  const classes = useStyles();
  const theme = useTheme();

  return (
    <Card className={classes.card}>
      <CopyToClipboard text={hexColor} onCopy={onCopy}>
        <Tooltip title="Copy">
          <IconButton
            className={classes.copyButton}
            style={{ color: theme.palette.getContrastText(hexColor) }}
          >
            <Icon icon={copyFill} width={20} height={20} />
          </IconButton>
        </Tooltip>
      </CopyToClipboard>

      <Box sx={{ bgcolor: hexColor, paddingTop: '56%' }} />

      <Box sx={{ p: 2.5 }}>
        <Box sx={{ typography: 'subtitle1', textTransform: 'capitalize' }}>
          {variation}
        </Box>
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 1, mt: 1.5 }}>
          <Box
            variant="overline"
            component={Typography}
            sx={{ color: 'text.secondary', width: 56 }}
          >
            Hex
          </Box>
          <Typography variant="body2">{hexColor}</Typography>
        </Box>
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <Box
            variant="overline"
            component={Typography}
            sx={{ color: 'text.secondary', width: 56 }}
          >
            Rgb
          </Box>
          <Typography variant="body2">
            {hexToRgb(hexColor)
              .replace('rgb(', '')
              .replace(')', '')}
          </Typography>
        </Box>
      </Box>
    </Card>
  );
}

function ColorsView() {
  const classes = useStyles();
  const theme = useTheme();
  // const { enqueueSnackbar } = useSnackbar();
  const [, setState] = useState(null);

  const onCopy = color => {
    setState(color);
    if (color) {
      // enqueueSnackbar(`Copied ${color}`, { variant: 'success' });
    }
  };

  return (
    <Page title="Foundations | Colors" className={classes.root}>
      <Container maxWidth="lg">
        <HeaderDashboard
          heading="Colors"
          links={[
            { name: 'Dashboard', href: PATH_APP.root },
            { name: 'Foundations', href: PATH_APP.foundations.root },
            { name: 'Colors' }
          ]}
          moreLink={[
            'https://next.material-ui.com/customization/color',
            'https://colors.eva.design'
          ]}
        />

        {COLORS.map(color => (
          <div key={color} className={classes.margin}>
            <Box
              component="h4"
              sx={{ typography: 'h4', textTransform: 'capitalize' }}
            >
              {color}
            </Box>

            <div className={classes.wrapper}>
              {COLORS_VARIATION.map(variation => (
                <ColorCard
                  key={variation}
                  variation={variation}
                  hexColor={theme.palette[color][variation]}
                  onCopy={() => onCopy(theme.palette[color][variation])}
                />
              ))}
            </div>
          </div>
        ))}

        <Box
          component="h4"
          sx={{ typography: 'h4', textTransform: 'capitalize' }}
        >
          Grey
        </Box>
        <div className={classes.wrapper}>
          {GREY_VARIATION.map(variation => (
            <ColorCard
              key={variation}
              variation={variation}
              hexColor={theme.palette.grey[variation]}
              onCopy={() => onCopy(theme.palette.grey[variation])}
            />
          ))}
        </div>
      </Container>
    </Page>
  );
}

export default ColorsView;
